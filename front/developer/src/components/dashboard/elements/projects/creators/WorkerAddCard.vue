<template lang="pug">
	transition(name="fade")
		v-card.worker-add-card(min-height="250", outlined)
			//v-img(height="125", src="/assets/images/lider-banner.png")
			v-card-title.text-center
				|Agregar personal
			v-card-text
				v-form(ref="workerForm", onSubmit="return false;")
					v-text-field.mb-4(
						label="Nombre *",
					...
					v-alert(color="orange darken-1", :value="!!alert", dismissible, transition="slide-y-transition")
						|{{ alert }}
			v-card-actions
				v-btn.add-worker-btn(absolute, fab, color="primary", @click="addWorker", :loading="status.loading")
					v-icon
						|mdi-plus
</template>
			...
			addWorker: async function() {
				...
				if( this.$refs.workerForm.validate() ){
					if( this.$refs.cv_file.files[0] ){
						...
						this.$projects.post(`add-worker/${this.$store.getters.currentProject._id}`, body)
						.then(result => {
							this.$store.commit("addProjectWorker", body)
							this.reset()
						})
						.catch(err => {
