				...
				let options = {
					...
					success: response => {
						...
						.then(result => {
							...
							if( this.$store.getters.currentProject.documents ){

								/*	Guarda el documento en el proyecto 	*/
								resource = "project/" + this.$store.getters.currentProject._id + "/add-document"
								...
								this.$refs.drgdrp.disable()
								this.$emit("upload")
								return this.$documents.post( resource, body, {headers: {Authorization: false}} )
							}
						})
						.catch(err => {
							this.$report(err, {swal: true})
