<template lang="pug">
	v-dialog(content-class="rate-project-dialog", persistent, :value="show", max-width="600")
		v-card
			v-card-title.mb-8
				|Ponderando: {{ $catalog[ doc.type ].name }}
			v-card-text.pb-0
				v-slider(
					:thumb-size="40",
					:color="color",
	...
	export default {
		...
		methods: {
			rate: function() {
				...
				this.$projects.post(`/project/${this._id}/document-rate/${this.doc.type}`, body)
				.then(result => this.$emit("rated", result.data.rate ) )
