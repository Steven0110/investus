	...
	.financiamiento
		v-row
			v-col(cols="12", md="4")
				v-card
					v-card-text
						v-text-field(:value="$store.getters.currentProject.goal.amount", type="number", label="Monto a financiar (MXN)" prepend-icon="mdi-currency-usd", outlined, @change="changeGoal", disabled)
						...
						v-row
							v-col(cols="6")
								.cobertura
									...
									v-chip(color="secondary")
										|{{ $store.getters.currentProject.goal.amount || 0 | currency }}
									.equals
										|=
							v-col(cols="6")
								.resultado
...
<script>
	...
	export default {
		...
		computed: {
			coverage: function() {
				return Number(parseFloat(this.warranty / (this.$store.getters.currentProject.goal.amount || this.warranty)).toFixed(2))
			},
			warranty: function() {
				let sum = 0
